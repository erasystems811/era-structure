...
export function AssessmentFlow({ business, layer1, observation, layer2, report, questions, staff, businessId }: Props) {
  ...
  if (step === 'report' && report) {
    ...
    const findings = (Array.isArray(c.key_findings) ? c.key_findings : []) as { headline?: string; evidence?: string; root_cause?: string; impact?: string; category?: string }[]
    const contradictions = (Array.isArray(c.contradiction_analysis) ? c.contradiction_analysis : []) as { owner_stated?: string; reality?: string }[]
